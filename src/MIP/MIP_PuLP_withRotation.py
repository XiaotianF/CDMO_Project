...
def create_solver(width, n_circuits, w_size, l_size, time_limit_seconds):
    ...
    R = [pulp.LpVariable(f'R{i}', cat='Binary') for i in range(n_circuits)] #R=0 means no rotation, R=1 means rotated
    # Or value
    ...
    # Constraints
    # Consider the rotation happens
    for i in range(n_circuits):
        for j in range(n_circuits):
            if i != j:
                    model += (x_pos[i] + (1 - R[i]) * w_size[i] + R[i] * l_size[i] <= x_pos[j] + overlap[i][j][0] * width)
                    model += x_pos[j] + (1 - R[i]) * w_size[j] + R[i] * l_size[j] <= x_pos[i] + overlap[j][i][0] * width
                    model += y_pos[i] + (1 - R[i]) * l_size[i] + R[i] * w_size[i] <= y_pos[j] + overlap[i][j][1] * width
                    model += y_pos[j] + (1 - R[i]) * l_size[j] + R[i] * w_size[j] <= y_pos[i] + overlap[j][i][1] * width
    ...
    for i in range(n_circuits):
        model += (x_pos[i] + (1-R[i]) * w_size[i] + R[i] * l_size[i] <= width)  # width constraint
        model += (y_pos[i] + (1-R[i]) * l_size[i] + R[i] * w_size[i] <= length)  # length constraint
        model += (0 <= x_pos[i])
        model += (0 <= y_pos[i])
...
def main():
    ...
    selected_files = ["ins-3.txt"]  # Choose the files you want to input, like "ins-1.txt" "ins-2.txt together
    output_folder = "./out"
